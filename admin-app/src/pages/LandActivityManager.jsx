...
import { Plus, Edit2, Trash2, Save, X, Eye, EyeOff, Library, MapPin, Clock, Search, ArrowUpDown, Calendar, TrendingUp, Map, Globe, SearchX, Trees } from 'lucide-react';
...
const LandActivityManager = () => {
 ...
 queryKey: ['activities', 'land'],
 queryFn: async () => {
 const { data, error } = await supabase
 .from('services')
 .select('*')
 .eq('category', 'Land Activities')
 ...
 onSuccess: () => {
 queryClient.invalidateQueries(['activities', 'land']);
 ...
 if (isLoading) return <div className="flex items-center justify-center py-20 animate-pulse text-red-500 font-black uppercase tracking-widest text-[10px]">Loading Land Activities...</div>;
 ...
 <div>
 <h1 className="text-3xl font-black text-gray-900 tracking-tight ">Land Activities</h1>
 <p className="text-gray-500 font-medium">Manage terrestrial excursions and land-based adventures</p>
 </div>
 <button
 ...
 <input
 type="text"
 placeholder="Search land activities by name, location or description..."
 ...
 {editingId ? 'Modify Expedition' : 'Deploy Land Experience'}
 </h2>
 <p className="text-lg text-slate-400 font-medium mt-1">Configure terrestrial experiences and environmental parameters</p>
 </div>
 </div>

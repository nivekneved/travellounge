...
const LandActivityManager = () => {
    ...
    const {
        ...
        filter: { category: 'activities' }, // Assuming filter for activities
        orderColumn: 'display_order',
        ascending: true,
        onSaveSuccess: () => resetForm()
    });

    // Note: The original code showed 'Land Activities' hardcoded but filtered from 'services' table likely.
    // I will filter for category: 'activities' and type: 'Land' if applicable.
    const landActivities = activities.filter(a => a.type === 'Land' || a.category === 'activities');
    ...
    const filteredActivities = landActivities.filter(activity => {
    ...
    const stats = [
        { label: 'Market Inventory', value: landActivities.length, icon: TrendingUp, color: 'text-red-600', bg: 'bg-red-50' },
        { label: 'Active Channels', value: landActivities.filter(a => a.is_active).length, icon: Globe, color: 'text-green-600', bg: 'bg-green-50' },
        { label: 'Geo Clusters', value: [...new Set(landActivities.map(a => a.location))].filter(Boolean).length, icon: MapPin, color: 'text-blue-600', bg: 'bg-blue-50' }
    ];
    ...
    return (
        <ManagerLayout
            title="Land Experiences"
            subtitle="Terrestrial Excursion & Expedition Architecture"
            icon={Trees}
            stats={stats}
            searchTerm={searchTerm}
            setSearchTerm={setSearchTerm}
            searchPlaceholder="SEARCH EXPEDITION REGISTRY..."
                ...
                <tr key={activity.id} className="transition-all hover:bg-slate-50/80 group align-middle">
                    ...
                    <td className="py-6 px-4">
                        <div className="flex flex-col">
                            <span className="font-black text-slate-900 text-lg tracking-tight group-hover:text-red-600 transition-colors uppercase leading-tight line-clamp-1">{activity.name}</span>
                    ...
                    <td className="py-6 px-4 text-center">
                        <div className={`inline-flex items-center px-4 py-1.5 rounded-full text-[9px] font-black uppercase tracking-[0.2em] border transition-all ${activity.is_active !== false ? 'bg-green-50 text-green-700 border-green-100' : 'bg-slate-100 text-slate-400 border-slate-200'}`}>
                            <span className={`w-2 h-2 rounded-full mr-2 ${activity.is_active !== false ? 'bg-green-500 animate-pulse' : 'bg-slate-400'}`}></span>
                ...
                <div className="space-y-12">
                    <section className="space-y-6">
                        <div className="flex items-center gap-3 mb-8">
                            ...
                            <h3 className="text-sm font-black text-slate-900 uppercase tracking-[0.2em]">Core Identity</h3>
                        </div>
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input type="text" value={formData.name} onChange={(e) => setFormData({ ...formData, name: e.target.value })}
                                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-4 focus:ring-red-600/10 focus:bg-white outline-none transition-all font-black text-slate-900 placeholder:text-slate-300" placeholder="e.g. Atlas Mountain Safari" />
                            </div>
                                ...
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" value={formData.location} onChange={(e) => setFormData({ ...formData, location: e.target.value })}
                                            className="w-full pl-14 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-900" placeholder="e.g. Marrakesh" />
                                    </div>
                                </div>
                                <div className="space-y-2">
                                    ...
                                    <div className="relative">
                                        ...
                                        <input type="text" value={formData.duration} onChange={(e) => setFormData({ ...formData, duration: e.target.value })}
                                            className="w-full pl-14 pr-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-900" placeholder="e.g. 4 Hours" />
                                    </div>
                                </div>
                            </div>
                            <div className="space-y-2">
                                ...
                                <textarea rows={4} value={formData.description} onChange={(e) => setFormData({ ...formData, description: e.target.value })}
                                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none font-medium text-slate-600 resize-none" placeholder="Describe the experience parameters..." />
                            </div>
                        </div>
                    </section>
                        ...
                        <div className="space-y-6">
                            <div className="space-y-2">
                                ...
                                <input type="number" value={formData.pricing.price} onChange={(e) => setFormData({ ...formData, pricing: { ...formData.pricing, price: e.target.value } })}
                                    className="w-full px-6 py-4 bg-slate-50 border border-slate-100 rounded-2xl font-black text-slate-900" placeholder="2500" />
                            </div>
                            <div className="space-y-2">
                            ...
                            <div className="flex items-center justify-between p-6 bg-slate-50 rounded-2xl border border-slate-100">
                                <div className="flex items-center gap-4">
                                    <div className={`p-2 rounded-lg ${formData.is_active ? 'bg-green-100 text-green-600' : 'bg-slate-200 text-slate-400'}`}><Globe size={18} /></div>
                                    <div>
                                ...
                                <button type="button" onClick={() => setFormData({ ...formData, is_active: !formData.is_active })} className={`w-14 h-8 rounded-full p-1 transition-all ${formData.is_active ? 'bg-green-600' : 'bg-slate-300'}`}>
            ...
            renderPreview={() => (
                <div className="w-full max-w-[340px] bg-white rounded-[32px] overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-500 m-auto mt-20">
                    <div className="h-[200px] relative overflow-hidden">
                        <img src={formData.images[0] || 'https://images.unsplash.com/photo-1464822759023-fed622ff2c3b?auto=format&fit=crop&w=800&q=80'} className="w-full h-full object-cover" alt="" />
                    ...
                    <div className="p-6 space-y-4">
                        <div>
                            ...
                            <div className="flex items-center gap-3 mt-2 text-slate-400">
                                <div className="flex items-center gap-1"><MapPin size={10} /><span className="text-[9px] font-black uppercase">{formData.location || 'N/A'}</span></div>
                        ...
                        <div className="pt-4 border-t border-slate-100 flex items-center justify-between">
                            ...
                            <div className="h-10 w-10 bg-red-600 text-white rounded-full flex items-center justify-center"><ArrowRight size={18} /></div>
                        </div>
                    </div>
                </div>
            )}
...
export default LandActivityManager;

...
const PageContentManager = () => {
 ...
 <span className="text-[10px] bg-green-100 text-green-700 font-black uppercase tracking-widest px-2 py-0.5 rounded-md border border-green-200">
 Status: Live
 </span>
 <span className="text-[10px] text-gray-400 font-mono">/{selectedPage?.slug}</span>
 </div>
 </div>
 </div>
 <button
 ...
 >
 {updateMutation.isPending ? <Loader size={18} className="animate-spin" /> : <Save size={18} />}
 Save Changes
 </button>
 </div>
 ...
 {['B', 'I', 'H1', 'H2', 'Link', 'List'].map(tool => (
 <button key={tool} type="button" className="w-8 h-8 flex items-center justify-center rounded-lg hover:bg-gray-200 text-gray-500 text-xs font-bold transition-colors">
 {tool}
 </button>
 ))}
 </div>
